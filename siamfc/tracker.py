...
class SiamFCTracker(Tracker):  #定义一个追踪器
    ...
    def __init__(self,model_path=None,cfg=None,group=2):
        ...
        self.SimClrLoss = SimClrLoss()
        self.criterion_con = torch.nn.CrossEntropyLoss().cuda()
    ...
    def train_step(self, batch, backward=True):
        ...
        with torch.set_grad_enabled(backward):
            ...
            zf = self.net.features(z)
            xf = self.net.features(x)
            zf_ = self.net.features(z_)
            responses = self.net.head(zf,xf)
            logits, labels_ = self.SimClrLoss(torch.cat([zf.view(8,-1), zf_.view(8,-1)], dim=0))
            loss_ = self.criterion_con(logits.cuda(), labels_.cuda())
