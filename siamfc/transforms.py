...
class SiamFCTransforms(object):
    ...
    def __init__(self, exemplar_sz=127, instance_sz=255, context=0.5):
        ...
        self.transforms_x = Compose([
            ...
            ToTensor()])
        self.transforms_z_ = transforms.Compose([
            # transforms.RandomResizedCrop(32),
            # transforms.RandomHorizontalFlip(p=0.5),
            transforms.RandomApply([transforms.ColorJitter(0.8, 0.8, 0.8, 0.2)], p=0.8),
            transforms.RandomGrayscale(p=0.2),
            transforms.ToTensor(),
            transforms.GaussianBlur(int(0.1 * 255)),
    ...
    def __call__(self, z, x, box_z, box_x):
        ...
        t = self.transforms_z(z + np.array(self.transforms_z_(Image.fromarray(z)), dtype = 'uint8').transpose(1, 2, 0))
